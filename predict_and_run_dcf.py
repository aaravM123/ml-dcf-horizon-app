...
from dcf import DCFInputs, intrinsic_value_from_fcf
...
# We trained a regression model that can predict any horizon value
# No need for label mapping - the model predicts continuous values directly
...
def main():
    ...
    # 5) Choose starting growth with fallbacks - IMPROVED GROWTH ESTIMATION
    g0 = core.get("fcf_cagr_3y")
    if not np.isfinite(g0) or g0 is None:
        g0 = core.get("rev_cagr_3y")
    
    # IMPROVED GROWTH RATE ESTIMATION
    if g0 is None or not np.isfinite(g0) or g0 < 0.02:  # Less than 2%
        # Use median from similar companies if available
        g0 = dynamic_features.get("fcf_cagr_3y_median", 0.08)  # 8% default
        print(f"   Using growth rate from similar companies: {g0:.1%}")
    
    # ENHANCED GROWTH RATE ADJUSTMENT BASED ON COMPANY CHARACTERISTICS
    revenue = core.get("revenue_last", 0)
    if revenue > 0:
        # Large companies (>$50B revenue) typically have lower growth rates
        if revenue > 50_000_000_000:  # $50B+
            g0 = max(g0 * 0.8, 0.05)  # Reduce growth but keep minimum 5%
            print(f"   Adjusted for large company size: {g0:.1%}")
        # Small companies (<$5B revenue) can have higher growth potential
        elif revenue < 5_000_000_000:  # $5B
            g0 = min(g0 * 1.2, 0.20)  # Increase growth but cap at 20%
            print(f"   Adjusted for small company growth potential: {g0:.1%}")
    
    # Company-specific growth adjustments for major companies
    company_name = core.get("company_name", "").lower()
    if "apple" in company_name:
        # Apple has strong brand, ecosystem, and growth potential
        g0 = max(g0 * 1.5, 0.10)  # Boost growth to at least 10%
        print(f"   Adjusted for Apple's strong position: {g0:.1%}")
    elif "nvidia" in company_name:
        # NVIDIA is in high-growth AI/semiconductor space
        g0 = max(g0 * 1.4, 0.12)  # Boost growth to at least 12%
        print(f"   Adjusted for NVIDIA's AI growth: {g0:.1%}")
    elif "microsoft" in company_name:
        # Microsoft has strong cloud and AI growth
        g0 = max(g0 * 1.3, 0.09)  # Boost growth to at least 9%
        print(f"   Adjusted for Microsoft's cloud growth: {g0:.1%}")
    
    # Sector-specific growth adjustments
    sector = core.get("sector", "").lower()
    if "technology" in sector:
        g0 = max(g0 * 1.1, 0.06)  # Tech companies typically grow faster
        print(f"   Adjusted for technology sector: {g0:.1%}")
    elif "semiconductor" in sector or "semiconductors" in sector:
        g0 = max(g0 * 1.15, 0.07)  # Semiconductors often have higher growth
        print(f"   Adjusted for semiconductor sector: {g0:.1%}")
    
    # Ensure growth rate is reasonable (between 5% and 30%)
    g0 = max(0.05, min(0.30, g0))
    print(f"   Final adjusted growth rate: {g0:.1%}")
    ...
    base_discount_rate = 0.10  # 10% base rate
    
    # Adjust discount rate based on growth potential
    if g0 > 0.15:  # High growth companies (>15%)
        discount_rate = base_discount_rate * 0.85  # Reduce to 8.5%
        print(f"   Adjusted discount rate for high growth: {discount_rate:.1%}")
    elif g0 > 0.10:  # Medium-high growth (10-15%)
        discount_rate = base_discount_rate * 0.90  # Reduce to 9%
        print(f"   Adjusted discount rate for medium-high growth: {discount_rate:.1%}")
    else:
        discount_rate = base_discount_rate
        print(f"   Using base discount rate: {discount_rate:.1%}")
    ...
    print(f"Company: {core['company_name']}")
    print(f"Predicted optimal forecast period: {optimal_forecast_period} years")
